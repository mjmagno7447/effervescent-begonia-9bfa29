...
for html_file in html_files:
    ...
    # Skip if hamburger JS already properly added (after image protection script)
    if 'const hamburgerBtn = document.getElementById' in content and 'img.addEventListener(\'dragstart\'' in content:
        print(f"Skipped {html_file} - hamburger JS already present")
        continue
    
    # If it has image protection script but no hamburger JS, add hamburger JS before closing body
    if 'img.addEventListener(\'dragstart\'' in content and 'const hamburgerBtn' not in content:
...
print("Hamburger JS fix complete!")
